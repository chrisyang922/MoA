...
def build_planner_prompt(query_text, profile_snippets):
    profile_block = "\n\n".join(profile_snippets) if profile_snippets else "(no past responses)"
    return f"""{_PLANNER_PROMPT}

Past user responses:
{profile_block}

Topic Generation and rating:
{query_text}

List 3–6 concise aspects the user is likely to mention (2–5 words each):"""
...
def build_retrieved_prompt_planpers(query, profile_snippets, plan_text):
   ...
   instruction = (
       "You are a helpful language model that assists in writing personalized topic responses.\n"
       "You are given several past writings written by the same individual. \n"
        "These writings are ordered from MOST RECENT to OLDEST. "
        "Focus primarily on the most recent writings at the beginning of the list, "
        "as they best reflect the writer's current tone, phrasing, and vocabulary. "
        "Maintain overall consistency with their general style but adapt most closely "
        "to how they write in the newest writings.\n\n"
        "Below are the writings by the user:\n"
       f"{profile_block}\n\n"
   ...
   task = (
       "Now write a NEW response for the following topic that matches the writer's style above.\n"
       "Do NOT include any reasoning, explanation, or repetition of the task.\n"
       "Do NOT directly copy full sentences from the previous writings.\n"
       "Do NOT include metadata, website elements, or UI text such as "
       "'Share', 'Report Abuse', 'Reply', etc.\n"
       "Start directly with the response text itself.\n\n"
       "End your response naturally once your main thought is complete. "
...
def build_moa_aggregator_prompt(query_text, profile_snippets, plan_text, candA, candB, candC, candD):
   ...
   if isinstance(plan_text, list):
        plan_block = "\n".join(f"- {p.strip()}" for p in plan_text if p.strip())
   elif isinstance(plan_text, str):
        plan_block = plan_text.strip()
   else:
        plan_block = "(no specific plan provided)"

   # This is for abstract generation dataset
   return (
       "You are a helpful language model that assists in writing personalized topic responses.\n"
       "You are given several past writings done by the same individual. "
        "These writings are ordered from MOST RECENT to OLDEST. "
        "Focus primarily on the most recent writings at the beginning of the list, "
        "as they best reflect the writer's current tone, phrasing, and vocabulary. "
        "Maintain overall consistency with their general style but adapt most closely "
       ...
       f"{profile_block}\n\n"
        ...
        f"PLAN FOR THE NEW REVIEW:\n{plan_block}\n\n"
       "Your goal is to synthesize multiple LLM-generated candidate responses into ONE final response that:\n"
       "- Preserves this user's tone, phrasing, and vocabulary as seen in their past response.\n"
       "- Combines the strongest ideas from the candidate responses.\n"
       "- Produces a fluent, coherent, and natural-sounding response.\n\n"
       "Now, below is the topic:"
       f"{query_text}"
       "You are also provided with candidate responses from different agents for the SAME topic:\n"
       "1. " + candA.strip() + "\n\n"
       "2. " + candB.strip() + "\n\n"
       "3. " + candC.strip() + "\n\n"
       "4. " + candD.strip() + "\n\n"
       "INSTRUCTIONS:\n"
       "1. Use the candidate responses as your main foundation, integrating their strongest ideas and expressions.\n"
       "2. You may introduce new phrasing or minor elaborations, but ONLY if they fit the writer's tone and vocabulary "
       "as reflected in the past writings.\n"
       "3. Maintain consistency with the writer's typical level of enthusiasm, detail, and formality.\n"
       "4. Resolve contradictions or repetition across candidates; keep the final response focused and non-redundant.\n"
       "5. Do NOT mention that these are LLM-generated responses or that you are combining multiple drafts.\n\n"
       "Return ONLY the final merged response text, starting directly with the response.\n"
   )
    ...
    try:
        for idx, (rid, query, profiles) in enumerate(rows, 1):
            ...
            if args.use_profile and profiles:
                ...
                picked_profiles = []
            #each agent generates its own plan
            cand_plans = []
            ...
            #layer 1, each agent uses its own plan
            cand_answers = []

            for j, mname in enumerate(cand_models):
                curr_plan = cand_plans[j]
                user_prompt = build_retrieved_prompt_planpers(query, picked_profiles, curr_plan)
                ...
                for ci in range(4):
                    if cand_writers[ci] is not None:
                        cand_writers[ci].write(
                            json.dumps(
                                {"author": rid, "model": cand_models[ci], "plan": cand_plans[ci], "output": (cand_answers[ci] or "").strip()},
                                ensure_ascii=False
                            ) + "\n"
                        )
            except Exception as e:
            ...
            if also_fout is not None:
                agg_prompt_l1 = build_moa_aggregator_prompt_planper_two(
                    query_text=query, profile_snippets=picked_profiles, plan_text=cand_plans,
                ...
                for ci in range(4):
                    if layer2_cand_writers[ci] is not None:
                        layer2_cand_writers[ci].write(
                            json.dumps(
                                {"author": rid, "model": l2_models[ci], "output": (l2_cand_answers[ci] or "").strip()},
                                ensure_ascii=False
                            ) + "\n"
                        )
            except Exception as e:
            ...
            if also_fout_l2 is not None:
                ...
                also_fout_l2.write(json.dumps({"author": rid, "output": fused_l2.strip()}, ensure_ascii=False) + "\n")
                ...
                for ci in range(4):
                    if layer3_cand_writers[ci] is not None:
                        layer3_cand_writers[ci].write(
                            json.dumps(
                                {"author": rid, "model": l3_models[ci], "output": (l3_cand_answers[ci] or "").strip()},
                                ensure_ascii=False
                            ) + "\n"
                        )
            except Exception as e:
            ...
            if also_fout_l3 is not None:
                ...
                also_fout_l3.write(json.dumps({"author": rid, "output": fused_l3.strip()}, ensure_ascii=False) + "\n")

            # ===== LAYER 4: PlanPers per candidate, using L3 drafts =====
                ...
                for ci in range(4):
                    if layer4_cand_writers[ci] is not None:
                        layer4_cand_writers[ci].write(
                            json.dumps(
                                {"author": rid, "model": l4_models[ci], "output": (l4_cand_answers[ci] or "").strip()},
                                ensure_ascii=False
                            ) + "\n"
                        )
            except Exception as e:
            ...
            if also_fout_l4 is not None:
                ...
                also_fout_l4.write(json.dumps({"author": rid, "output": fused_l4.strip()}, ensure_ascii=False) + "\n")

            # ===== LAYER 5: PlanPers per candidate, using L4 drafts =====
