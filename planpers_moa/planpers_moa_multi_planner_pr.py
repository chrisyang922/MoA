...
def build_retrieved_prompt_planpers(query, profile_snippets, plan_text):
   ...
   instruction = (
        ...
        "You are given several past reviews written by a user. \n"
        "Below are the review texts by the user:\n"
       f"{profile_block}\n\n"
...
def build_moa_aggregator_prompt(query_text, profile_snippets, plan_text, candA, candB, candC, candD):
   ...
   return (
       ...
       "- Produces a fluent, coherent, and natural-sounding reviews.\n\n"
       "Now, below is the product information:"
       f"{query_text}"
        "You are also provided with candidate reviews from different agents for the SAME product:\n"
        "1. " + candA.strip() + "\n\n"
        "2. " + candB.strip() + "\n\n"
        "3. " + candC.strip() + "\n\n"
        "4. " + candD.strip() + "\n\n"
...
def main():
    ...
    try:
        for idx, (rid, query, profiles) in enumerate(rows, 1):
            ...
            print("\n================= PLANS FOR CURRENT EXAMPLE =================")
            print(cand_plans)
            print("============================================================\n")

            #layer 1, uses its own plan
            cand_answers = []
                ...
                for ci in range(4):
                    if cand_writers[ci] is not None:
                        cand_writers[ci].write(
                            json.dumps(
                                {"reviewerId": rid, "model": cand_models[ci],"plan": cand_plans[ci],  "output": (cand_answers[ci] or "").strip()},
                                ensure_ascii=False
                            ) + "\n"
                        )
            except Exception as e:
