...
AGG_SYSTEM_Prompt = (
    "You are the Aggregator agent in a mixture-of-agents system for personalized research abstracts. "
...
that this researcher would typically include in their academic abstracts.
...
# Your output:
    ...
    evaluation-focused narrative
    multi-step experimental framing
...
def build_retrieved_prompt(query, profile_snippets, plan_text):
    if profile_snippets:
        cleaned_snippets = [
...
Your primary goal is to make the abstract sound exactly like this researcher’s prior writing.
Match:
- tone and formality
- sentence structure and flow
- typical intro→method→results→conclusion pattern
- preferred terminology density
If a trade-off exists, prioritize replicating the researcher’s writing voice.
"""

_SYSTEM_ROLE_COVERAGE = """
You are the Aspect-Coverage agent.
Your primary goal is to cover as many of the provided stylistic and content aspects as possible.
Treat the aspects list as a checklist.
Be thorough in touching all key elements while staying fluent and coherent.
"""

_SYSTEM_ROLE_ANALYTICAL = """
You are the Analytical agent.
Your goal is to emphasize clarity of contribution, problem framing, methodological rigor,
and evaluation structure — mirroring how the researcher typically justifies their work.
"""

_SYSTEM_ROLE_PRACTICAL = """
You are the Practical Implications agent.
Your goal is to describe intended applications, impact, and why the work matters,
in the same style and framing used in the researcher’s prior abstracts.
"""

def build_moa_aggregator_prompt(query_text, profile_snippets, plan, candA, candB, candC, candD):
   profile_block = "\n".join(" ".join(snippet.split()[:100]) for snippet in profile_snippets) if profile_snippets else "(no profile)"
   plan_block = plan.strip() if plan else "(no specific plan provided)"
   # This is for abstract generation dataset
   return (
        "You are a helpful language model that assists in writing personalized research abstracts.\n"
...
def load_llm(model_name):
    ...
    if "deepseek" in name or "r1-0528" in name:
        ...
        return {
            "client": client,
            "deployment": os.getenv("DEEPSEEK_DEPLOYMENT", "DeepSeek-R1-0528"),
        }, None, "azure-deepseek"
...
def main():
    ...
    ap.add_argument("--layer3_agg_model_name", type=str, default=None)
    ...
    ap.add_argument("--layer4_agg_model_name", type=str, default=None)
    ...
    for i, ex in enumerate(raw_rows, 1):
        rid = ex.get("name", i) if isinstance(ex, dict) else i
    ...
    def flush_batch():
        ...
        for rid, txt in zip(batch_ids, texts):
            ...
            fout.write(json.dumps({"name": rid, "output": txt.strip()}, ensure_ascii=False) + "\n")
        ...
        for idx, (rid, query, profiles) in enumerate(rows, 1):
            ...
            if also_fout is not None:
                ...
                agg_messages = [
                    ...
                    {"role": "user", "content": agg_prompt_l1},
                ]
                ...
                also_fout.write(json.dumps({"name": rid, "output": fused_l1.strip()}, ensure_ascii=False) + "\n")
            ...
            if args.also_agg_l2_out:
                ...
                with open(args.also_agg_l2_out, "a", encoding="utf-8") as f2:
                    f2.write(json.dumps({"name": rid, "output": fused_l2}, ensure_ascii=False) + "\n")
            ...
            if args.also_agg_l3_out:
                ...
                with open(args.also_agg_l3_out, "a", encoding="utf-8") as f3:
                    f3.write(json.dumps({"name": rid, "output": fused_l3}, ensure_ascii=False) + "\n")
            ...
            if args.also_agg_l4_out:
                ...
                with open(args.also_agg_l4_out, "a", encoding="utf-8") as f4:
                    f4.write(json.dumps({"name": rid, "output": fused_l4}, ensure_ascii=False) + "\n")
            ...
            if args.batch_size > 1:
                ...
                fout.write(json.dumps({"name": rid, "output": fused.strip()}, ensure_ascii=False) + "\n")
