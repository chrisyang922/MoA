    ...
    try:
        for idx, (rid, query, profiles) in enumerate(rows, 1):
                ...
                for ci in range(4):
                    if cand_writers[ci] is not None:
                        cand_writers[ci].write(
                            json.dumps(
                                {"name": rid, "model": cand_models[ci], "output": (cand_answers[ci] or "").strip()},
                                ensure_ascii=False
                            ) + "\n"
                        )
            except Exception as e:
                ...
                for ci in range(4):
                    if layer2_cand_writers[ci] is not None:
                        layer2_cand_writers[ci].write(
                            json.dumps(
                                {"name": rid, "model": l2_models[ci], "output": (l2_cand_answers[ci] or "").strip()},
                                ensure_ascii=False
                            ) + "\n"
                        )
            except Exception as e:
            ...
            if also_fout_l2 is not None:
                ...
                also_fout_l2.write(json.dumps({"name": rid, "output": fused_l2.strip()}, ensure_ascii=False) + "\n")
                ...
                for ci in range(4):
                    if layer3_cand_writers[ci] is not None:
                        layer3_cand_writers[ci].write(
                            json.dumps(
                                {"name": rid, "model": l3_models[ci], "output": (l3_cand_answers[ci] or "").strip()},
                                ensure_ascii=False
                            ) + "\n"
                        )
            except Exception as e:
            ...
            if also_fout_l3 is not None:
                ...
                also_fout_l3.write(json.dumps({"name": rid, "output": fused_l3.strip()}, ensure_ascii=False) + "\n")
                ...
                for ci in range(4):
                    if layer4_cand_writers[ci] is not None:
                        layer4_cand_writers[ci].write(
                            json.dumps(
                                {"name": rid, "model": l4_models[ci], "output": (l4_cand_answers[ci] or "").strip()},
                                ensure_ascii=False
                            ) + "\n"
                        )
            except Exception as e:
            ...
            if also_fout_l4 is not None:
                ...
                also_fout_l4.write(json.dumps({"name": rid, "output": fused_l4.strip()}, ensure_ascii=False) + "\n")
                ...
                for ci in range(4):
                    if layer5_cand_writers[ci] is not None:
                        layer5_cand_writers[ci].write(
                            json.dumps(
                                {"name": rid, "model": l5_models[ci], "output": (l5_cand_answers[ci] or "").strip()},
                                ensure_ascii=False
                            ) + "\n"
                        )
            except Exception as e:
            ...
            if args.batch_size > 1:
                ...
                fout.write(json.dumps({"name": rid, "output": fused_l5.strip()}, ensure_ascii=False) + "\n")
