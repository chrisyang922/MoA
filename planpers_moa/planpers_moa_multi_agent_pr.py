...
_PLANNER_PROMPT = """You are a helpful assistant that extracts the main aspects a reviewer is likely to discuss in a product review.
Given a product description and the user's previous reviews, identify 3–6 concise, specific aspects
that this reviewer would probably comment on (e.g., build quality, sound clarity, comfort, ease of installation).
...
# Your task:
    List only short aspect phrases (2–5 words each) that capture what this reviewer tends to focus on.
...
def build_planner_prompt(query_text, profile_snippets):
    profile_block = "\n\n".join(profile_snippets) if profile_snippets else "(no past reviews)"
...
# ROLE-SPECIFIC SYSTEM PROMPTS  (L1 only)
_SYSTEM_ROLE_VOICE = """
You are the Voice & Tone agent.
Your primary goal is to make the review sound exactly like it was written by this user.
Match:
- tone
- phrasing
- sentiment
- enthusiasm level
- typical structure
If there is a trade-off, prioritize the user's voice over covering all aspects.
"""

_SYSTEM_ROLE_COVERAGE = """
You are the Aspect-Coverage agent.
Your primary goal is to cover as many provided aspects as possible.
Treat the aspects list as a checklist.
Be thorough and complete.
"""

_SYSTEM_ROLE_ANALYTICAL = """
You are the Analytical & Contrastive agent.
Your primary goal is to provide clear reasoning, pros/cons, comparisons, and trade-offs.
Explain what works well and what doesn’t.
"""

_SYSTEM_ROLE_PRACTICAL = """
You are the Practical & Actionable agent.
Your primary goal is to focus on real-world usage, scenarios, and practical implications.
Describe how the product would be used and in what contexts it shines or struggles.
"""


def build_retrieved_prompt(query_text: str, profile_snippets: list[str], plan_text) -> str:
    """
    Build the USER prompt for personalized product review generation using RAG.

    Args:
        query_text: Description of the new product (can include rating and metadata).
        profile_snippets: List of past review texts for this user, ideally ordered
                          from most recent to oldest.

    Returns:
        A complete user prompt string (to be used with different SYSTEM role prompts).
    """
    if profile_snippets:
        # Truncate each snippet for length, then join with spacing
        cleaned_snippets = [
            " ".join(snippet.split()[:100]) for snippet in profile_snippets
        ]
        profile_block = "\n\n".join(cleaned_snippets)
    else:
        profile_block = "(no past reviews available for this user)"

    instruction = (
        "You are a helpful language model that assists in writing personalized product reviews.\n"
        "You are given several past reviews written by a single user.\n"
        "These reviews are ordered from MOST RECENT to OLDEST. "
        "Focus primarily on the most recent reviews at the beginning of the list, "
        "as they best reflect the reviewer's current tone, phrasing, and vocabulary. "
        "Maintain overall consistency with their general style, but adapt most closely "
        "to how they write in the newest reviews.\n\n"
        "Below are the review texts by this user:\n"
        f"{profile_block}\n\n"
        "Below is a high-level PLAN describing which aspects to focus on in the new review that you will be generating.\n"
        "Follow this plan for WHAT to mention and roughly HOW to structure the review. \n"
        f"PLAN FOR THE NEW REVIEW:\n{plan_text}\n\n"
    )
    ...
    task = (
        ...
        "Do NOT directly copy full sentences from the previous reviews.\n"
        "Do NOT include metadata, website elements, or UI text such as "
        "'Share', 'Report Abuse', 'Reply', etc.\n"
        "Start directly with the review text itself.\n\n"
        "End your review naturally once your main thought is complete. "
...
def build_moa_aggregator_prompt(
    ...
    """
    Build the USER prompt for the MoA aggregator that fuses 4 candidate reviews
    into a single personalized review, using RAG from user profile.
    """
    if profile_snippets:
        cleaned_snippets = [
            " ".join(snippet.split()[:100]) for snippet in profile_snippets
    ...
    return (
        "You are a helpful language model that assists in writing personalized product reviews.\n"
        "Your goal is to synthesize multiple LLM-generated candidate reviews into ONE final review that:\n"
        "- Preserves this user's tone, phrasing, and vocabulary as seen in their past reviews.\n"
        "- Combines the strongest ideas from the candidate reviews.\n"
        "- Produces a fluent, coherent, and natural-sounding review.\n\n"
        f"Product information:\n{query_text}\n\n"
        "Past reviews written by this user (most recent first):\n"
        f"{profile_block}\n\n"
        "Below is a high-level PLAN describing which aspects to focus on in the new review that you will be generating.\n"
        "Follow this plan for WHAT to mention and roughly HOW to structure the review. \n"
        f"PLAN FOR THE NEW REVIEW:\n{plan}\n\n"
        "Candidate reviews from different agents for the SAME product:\n"
        "1. " + candA.strip() + "\n\n"
        "2. " + candB.strip() + "\n\n"
        "3. " + candC.strip() + "\n\n"
        "4. " + candD.strip() + "\n\n"
        "INSTRUCTIONS:\n"
        "1. Use the candidate reviews as your main foundation, integrating their strongest ideas and expressions.\n"
        "2. You may introduce new phrasing or minor elaborations, but ONLY if they fit the user's tone and vocabulary "
        "as reflected in the past reviews.\n"
        "3. Maintain consistency with the user's typical level of enthusiasm, detail, and formality.\n"
        "4. Resolve contradictions or repetition across candidates; keep the final review focused and non-redundant.\n"
        "5. Do NOT mention that these are LLM-generated reviews or that you are combining multiple drafts.\n\n"
        "Return ONLY the final merged review text, starting directly with the review.\n"
    )


system_prompt = (
    "You are a helpful language model that writes personalized product reviews. "
    "Always follow the instructions in the user message and write fluent, natural reviews."
)

AGG_SYSTEM_Prompt = (
    "You are the Aggregator agent in a mixture-of-agents system for personalized product reviews. "
    "You specialize in combining multiple candidate reviews into a single, high-quality personalized review."
)
...
def main():
    ...
    for i, ex in enumerate(raw_rows, 1):
        rid = ex.get("reviewerId", i) if isinstance(ex, dict) else i
    ...
    def flush_batch():
        ...
        for rid, txt in zip(batch_ids, texts):
            ...
            fout.write(json.dumps({"reviewerId": rid, "output": txt.strip()}, ensure_ascii=False) + "\n")
    ...
    try:
        for idx, (rid, query, profiles) in enumerate(rows, 1):
            ...
            if also_fout is not None:
                ...
                also_fout.write(json.dumps({"reviewerId": rid, "output": fused_l1.strip()}, ensure_ascii=False) + "\n")
            ...
            if args.also_agg_l2_out:
                ...
                with open(args.also_agg_l2_out, "a", encoding="utf-8") as f2:
                    f2.write(json.dumps({"reviewerId": rid, "output": fused_l2}, ensure_ascii=False) + "\n")
            ...
            if args.also_agg_l3_out:
                ...
                with open(args.also_agg_l3_out, "a", encoding="utf-8") as f3:
                    f3.write(json.dumps({"reviewerId": rid, "output": fused_l3}, ensure_ascii=False) + "\n")
            ...
            if args.also_agg_l4_out:
                ...
                with open(args.also_agg_l4_out, "a", encoding="utf-8") as f4:
                    f4.write(json.dumps({"reviewerId": rid, "output": fused_l4}, ensure_ascii=False) + "\n")
            ...
            if args.batch_size > 1:
                ...
                fout.write(json.dumps({"reviewerId": rid, "output": fused.strip()}, ensure_ascii=False) + "\n")
